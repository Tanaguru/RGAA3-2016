...
All the `<img>` tags of the page not within a link not identified as captcha (see Notes about captcha detection) (css selector : `img:not(a img):not([longdesc])`) and the `"role"` attribute that value is different than "img" or "presentation" is missing.
